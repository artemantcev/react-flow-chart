...
var PortWrapper = /** @class */ (function (_super) {
    ...
    function PortWrapper() {
        ...
        _this.onMouseDown = function (startEvent) {
            ...
            var mouseMoveHandler = function (e) {
                var _a = _this.context, offsetX = _a.offsetX, offsetY = _a.offsetY, zoomScale = _a.zoomScale;
                onLinkMove({
                    config: config,
                    linkId: linkId,
    ...
    PortWrapper.prototype.componentDidMount = function () {
    ...
    PortWrapper.prototype.componentDidUpdate = function (prevProps) {
        ...
        if (this.portsOfType(this.props) !== this.portsOfType(prevProps)
            || !lodash_1.isEqual(this.props.node.size, prevProps.node.size)
            || !lodash_1.isEqual(this.props.portsSize, prevProps.portsSize)) {
            this.updatePortPosition();
        }
    };
    PortWrapper.prototype.render = function () {
        ...
        return (React.createElement("div", { "data-port-id": port.id, "data-node-id": node.id, onMouseDown: this.onMouseDown, ref: this.nodeRef, style: style },
            React.createElement(Component, { config: config, node: node, port: port, isSelected: !!selected && selected.type === 'port' && selected.id === port.id, isHovered: !!hovered && hovered.type === 'port' && hovered.id === port.id, isLinkSelected: selectedLink
    ...
    PortWrapper.prototype.portsOfType = function (props) {
        var type = props.port.type, ports = props.node.ports;
        return Object.values(ports).reduce(function (count, port) { return port.type === type ? count + 1 : count; }, 0);
    };
